...
import com.ludoelite.service.WebSocketService;
...
public class LudoBoardController extends BaseController {
     ...
     * This is called on a background thread, so we use Platform.runLater.
     */
     ...
     */
    private void applyMoveFromServer(PieceMove moveData) {
    ...
    private void handleRollDice() {
        if (gameEngine.getGameState() != GameState.IN_PROGRESS) {
            AlertHelper.showWarning("Game Not Active", 
        ...
        if (result == -1) {
            AlertHelper.showWarning("Invalid Roll", 
    ...
    /**
     * Finds a piece at the given canvas coordinates.
     */
    private GamePiece findPieceAt(double x, double y, Player player) {
        // Check each piece to see if click is within its bounds
        for (GamePiece piece : player.getPieces()) {
            Position pos = getPieceCanvasPosition(piece);
            if (pos != null) {
    ...
    /**
     * Helper to get piece position on canvas.
     * This duplicates some logic from BoardRenderer for click detection.
     */
    private Position getPieceCanvasPosition(GamePiece piece) {
        // Simplified - in production, BoardRenderer should expose this
        // For now, return null and pieces can be selected by clicking anywhere
        return null;
    }
